...
# Delete this year's states after successful run completion
state_delete_year=$1
shift
...
# Delete states that this run started from
if [[ -d "${state_path_absolute}/${state_delete_year}" ]]; then
    rm "${state_path_absolute}/${state_delete_year}"/*.state
fi
