...
for thisSSP in ${ssp_list}; do
    ...
    state_path=""
    state_path_absolute=$(g2p_get_state_path_absolute.sh "${rundir_top}" "${state_path_absolute}" ${istest})
    ...
    if [[ ${actual_only} -eq 0 ]]; then
        ...
        if [[ ${sequential_pot} -eq 0 ]]; then
            if [[ ${potential_only} -eq 1 ]]; then
                ...
                lastactrun=$(grep "LPJ-GUESS" ${HOME}/submitted_jobs.log | tail -n 1 | grep -oE "[0-9]+")
