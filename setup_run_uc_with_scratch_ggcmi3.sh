...
script="setup_run_uc_with_scratch_ggcmi3.sh"
...
if [[ "${state_path_absolute}" == "" ]]; then
	topdir=$(realpath ..)
	state_path_absolute=$(echo $topdir | sed "s@/pfs/data6@@" | sed "s@$HOME@$WORK@" | sed "s@/$topdir@@")/states
